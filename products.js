...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function displayProducts(selectedCategories) {
    ...
    // Lấy dữ liệu sản phẩm thật từ localStorage (do Admin quản lý)
    let allMasterProducts = getData("products") || [];

    if (allMasterProducts.length === 0) {
    console.warn("Lưu ý: localStorage 'products' đang trống. Trang User sẽ không hiển thị sản phẩm.");
    }
    let filteredProduct = [...allMasterProducts];
    ...
    if (!selectedCategories.includes("all")) {
      ...
      filteredProduct = filteredProduct.filter((product) => {
        ...
        return selectedCategories.includes(product.brand);
      });
    }
    // LỌC THEO GIÁ
    if (
  ...
  function handleSearch() {
    ...
    window.spaRouter.navigateToView("products");
  ...
  document.addEventListener("click", (e) => {
    if (
      e.target.closest(".products__list__item--img__cart, .product-hottest-item--img__cart")
    ) {
      return; // Để cho user.js xử lý
      }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (link.classList.contains("product-link")) {
      ...
      if (productId) { // Chỉ cần check có ID
        displayProductDetails(productId); // Truyền ID (dạng string)
      }
  return false;
}
    ...
    window.spaRouter.navigateToView(viewName);
...
function displayProductDetails(productId) {
  const allMasterProducts = getData("products") || [];
  const product = allMasterProducts.find((p) => String(p.id) === String(productId));
  if (!product) {
    return;
  }
  ...
  // Hiển thị view
  if (window.spaRouter && typeof window.spaRouter.navigateToView === "function") {
    window.spaRouter.navigateToView("product-details");
...
function setupAddToCartButton(product) {
  ...
  btn.addEventListener("click", function (e) {
    ...
    if (typeof addToCart === "function") {
      addToCart(product.id, productName, price, image, quantity);
    }
  });
      ...
      cart = [
        {
          ...
          name: product.name,
          price: price,
          image: product.img, // Sửa từ product.imgSrc
          quantity: quantity,
        },
      ];
...
function getDefaultDescription(product) {
  const { brand: category, name } = product;
  const descriptions = {
...
function updateBreadcrumb(product) {
  const categoryName = getCategoryName(product.brand);
