      ...
      if (mode === "users") {
        ...
        `).then(() => {
          ...
          document.getElementById("order-table-container").addEventListener("click", (e) => {
            ...
            const order = (getData("orders") || []).find(o => o.id == orderId); // Lấy data mới nhất
            ...
            `, (vals) => {
              ...
              if (productInStock) {
                ...
                if (newStatus === "Đã hủy" && oldStatus !== "Đã hủy") {
                  ...
                  alert(`Đã hoàn trả ${order.quantity} sản phẩm "${productInStock.name}" vào kho.`);
                ...
                // Nếu đổi từ "Đã hủy" sang trạng thái khác -> TRỪ KHO LẠI
                  ...
                  if (productInStock.quantity < order.quantity) {
                      alert(`Không thể đổi trạng thái! Tồn kho của "${productInStock.name}" không đủ.`);
                      return; // Không lưu
                  }
                  productInStock.quantity -= order.quantity;
                  alert(`Đã trừ ${order.quantity} sản phẩm "${productInStock.name}" khỏi kho.`);
                  setData("products", allProducts);
                  channel.postMessage({ type: 'products_updated' });
                }
              }
