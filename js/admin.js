...
function initOrdersPage() {
  ...
  function loadMode(mode) {
    ...
    if (mode === "users") {
      ...
      animateSidebarChange(
          ...
          btn.addEventListener("click", () => {
            ...
            showPopup(
              ...
              `
                ...
                <select name="role">
                  <option ${
                    u.role === "Khách hàng" ? "selected" : ""
                  }>Khách hàng</option>
                  <option ${
                    u.role === "Nhân viên" ? "selected" : ""
                  }>Nhân viên</option>
                </select>
                <select name="state">
        ...
        document.querySelectorAll(".delete-btn").forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            if (userIndex === -1)
              return alert("Lỗi: Không tìm thấy người dùng!");

            showPopup(
              `Bạn có chắc muốn xóa ${users[userIndex].name}?`,
              ``,
              (vals) => {
      ...
      if (
        allOrders.length > 0 &&
        typeof allOrders[0].productId === "undefined"
      ) {
      ...
      function renderOrderTable(ordersToRender) {
        ...
        if (ordersToRender.length === 0) {
          tableContainer.innerHTML =
            "<p style='text-align:center; color: #999; padding-top: 20px;'>Không tìm thấy đơn hàng nào.</p>";
          return;
        }
      ...
      animateSidebarChange(
        ...
        document
          ...
          .addEventListener("click", (e) => {
            ...
            const order = (getData("orders") || []).find(
              (o) => o.id == orderId
            ); // Lấy data mới nhất
            ...
            showPopup(
              ...
              <p style="color: #ccc;">Sản phẩm: <strong>${productName}</strong></p>
              <p style="color: #ccc;">Số lượng: <strong>${
                order.quantity
              }</strong></p>
              <p style="color: #ccc;">Tổng tiền: <strong>${formatCurrency(
                order.total
              )}</strong></p>
              ...
              <select name="status" style="width: 100%; padding: 8px; border-radius: 4px; border: none; background: #3a3a3a; color: white;">
                <option value="Mới đặt" ${
                  oldStatus === "Mới đặt" ? "selected" : ""
                }>Mới đặt</option>
                <option value="Đang xử lý" ${
                  oldStatus === "Đang xử lý" ? "selected" : ""
                }>Đang xử lý</option>
                <option value="Đã giao" ${
                  oldStatus === "Đã giao" ? "selected" : ""
                }>Đã giao</option>
                <option value="Đã hủy" ${
                  oldStatus === "Đã hủy" ? "selected" : ""
                }>Đã hủy</option>
              </select>
            `,
              (vals) => {
                ...
                if (productInStock) {
                  // Nếu hủy đơn (và trước đó không phải là "Đã hủy") -> HOÀN KHO
                  if (newStatus === "Đã hủy" && oldStatus !== "Đã hủy") {
                    productInStock.quantity += order.quantity;
                    alert(
                      `Đã hoàn trả ${order.quantity} sản phẩm "${productInStock.name}" vào kho.`
                    );
                    setData("products", allProducts);
                    channel.postMessage({ type: "products_updated" });
                  }
                  // Nếu đổi từ "Đã hủy" sang trạng thái khác -> TRỪ KHO LẠI
                  else if (newStatus !== "Đã hủy" && oldStatus === "Đã hủy") {
                    if (productInStock.quantity < order.quantity) {
                      alert(
                        `Không thể đổi trạng thái! Tồn kho của "${productInStock.name}" không đủ.`
                      );
                      return; // Không lưu
                    }
                    productInStock.quantity -= order.quantity;
                    alert(
                      `Đã trừ ${order.quantity} sản phẩm "${productInStock.name}" khỏi kho.`
                    );
                    setData("products", allProducts);
                    channel.postMessage({ type: "products_updated" });
                  }
                ...
                // Lưu lại đơn hàng (allOrders đã được cập nhật)
        ...
        document.querySelectorAll(".list_scrollbar").forEach((btn) => {
          btn.addEventListener("click", (e) => {
              ...
              showPopup(
                ...
                (vals) => {
                  ...
                  if (product.quantity < quantity) {
                    return alert(
                      `Không đủ hàng! Tồn kho của "${product.name}" chỉ còn ${product.quantity}.`
                    );
                  }
                  ...
                  const newOrder = {
                    ...
                    product: product.name, // Lưu tên để hiển thị
                    quantity: quantity,
                    total: product.price * quantity, // Tự động tính tổng tiền
                    status: vals.status,
                  };
                  ...
                  applyFiltersAndRender();
                  popup.classList.remove("active");
                  channel.postMessage({ type: "products_updated" }); // Gửi tín hiệu
                }
              );
            }
            ...
            if (act === "del") {
              const id = prompt(
                "Nhập ID đơn hàng muốn xóa (Hành động này KHÔNG hoàn kho):"
              );
              if (!id) return;
              ...
              if (idx >= 0) {
                if (
                  confirm(`Bạn có chắc muốn xóa vĩnh viễn đơn hàng #${id}?`)
                ) {
  ...
  channel.onmessage = (event) => {
    ...
    console.log(
      "Phát hiện cập nhật sản phẩm... Tự động làm mới các tab liên quan."
    );
    ...
    if (ordersPage && ordersPage.style.display === "block") {
      ...
      const activeModeBtn = ordersPage.querySelector(
        ".button_linear.active-mode"
      );
      if (activeModeBtn) {
