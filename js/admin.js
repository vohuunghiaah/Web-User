      ...
      if (mode === "users") {
        ...
        `).then(() => {
          ...
          document.getElementById("order-table-container").addEventListener("click", (e) => {
            ...
            const product = (getData("products") || []).find(p => p.id === order.productId);
            ...
            showPopup(`Chi tiết đơn hàng #${order.id}`, `
              ...
              <p style="color: #ccc;">Số lượng: <strong>${order.quantity}</strong></p>
              <p style="color: #ccc;">Tổng tiền: <strong>${formatCurrency(order.total)}</strong></p>
            ...
            `, (vals) => {
              ...
              const productInStock = allProducts.find(p => p.id === order.productId);
              
              if (productInStock) {
                let stockChanged = false; // Biến để kiểm tra xem có cần lưu kho không

                // KỊCH BẢN 1: HOÀN THÀNH ĐƠN (Trừ kho)
                // (Chỉ trừ khi đổi TỪ trạng thái KHÁC "Đã giao" SANG "Đã giao")
                if (newStatus === "Đã giao" && oldStatus !== "Đã giao") {

                  if (productInStock.quantity < order.quantity) {
                    alert(`Không thể hoàn thành! Tồn kho của "${productInStock.name}" không đủ (còn ${productInStock.quantity}).`);
                    return; // Không lưu, không đóng popup
                  }

                  productInStock.quantity -= order.quantity;
                  alert(`Đã xác nhận giao. Trừ ${order.quantity} sản phẩm "${productInStock.name}" khỏi kho.`);
                  stockChanged = true;
                }

                // KỊCH BẢN 2: HỦY ĐƠN HÀNG ĐÃ GIAO (Hoàn kho)
                // (Chỉ hoàn kho khi đổi TỪ "Đã giao" SANG "Đã hủy")
                else if (newStatus === "Đã hủy" && oldStatus === "Đã giao") {
                  productInStock.quantity += order.quantity;
                  alert(`Đã hoàn trả ${order.quantity} sản phẩm "${productInStock.name}" vào kho do hủy đơn đã giao.`);
                  stockChanged = true;
                }

                // (Các trường hợp khác, ví dụ: "Mới đặt" -> "Đã hủy", hoặc "Mới đặt" -> "Đang xử lý" sẽ không ảnh hưởng đến kho)

                // Nếu có thay đổi tồn kho, lưu lại và gửi tín hiệu
                if (stockChanged) {
                  setData("products", allProducts);
                  // Gửi tín hiệu để các trang khác (Dashboard, Products) tự làm mới
