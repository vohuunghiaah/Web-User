...
document.addEventListener("click", function (e) {
  ...
  // 1. LẤY ID SẢN PHẨM (QUAN TRỌNG NHẤT)
  const id = item.dataset.productId; 
  if (!id) {
    console.error("Không tìm thấy data-product-id trên item");
    return; 
  }

  // 2. LẤY CÁC THÔNG TIN CÒN LẠI
  ...
  // 3. GỌI HÀM addToCart VỚI 5 THAM SỐ (ĐÚNG THỨ TỰ)
...
function checkoutOrder() {
  ...
  for (const itemInCart of cart) {
    const productInStock = allProducts.find(p => p.id === itemInCart.id);

    if (productInStock && productInStock.quantity >= itemInCart.quantity) {
    } else {
      stockWasSufficient = false;
      alert(`Xin lỗi, sản phẩm "${itemInCart.name}" không đủ hàng (chỉ còn ${productInStock ? productInStock.quantity : 0})!`);
      break;
    }
  }

  if (!stockWasSufficient) {
    return; // Không cho thanh toán
  }
  ...
  const customerName = form.querySelector('input[placeholder="Họ và tên"]').value;

for (const itemInCart of cart) {
  const newOrder = {
    id: Date.now() + '-' + itemInCart.id, // ID duy nhất
    date: orderDate,
    user: customerName,
    ...
    status: "Mới đặt" // Trạng thái mặc định
  };
...
billProducts.innerHTML = ""; // Xóa nội dung cũ

// Lấy lại giỏ hàng (trước khi bị xóa) để hiển thị
const cartForBill = JSON.parse(localStorage.getItem("cart")) || []; 

cartForBill.forEach((item) => {
...
dateEl.innerText = new Date(orderDate).toLocaleString('vi-VN'); // Dùng orderDate đã tạo
...
function renderOrderHistory() {
  ...
  allOrders.forEach((order) => {
    // Cấu trúc mới: mỗi order là 1 sản phẩm
    const tr = document.createElement("tr");
    tr.innerHTML = `<td>${order.product} (x${order.quantity})</td>
                    <td>${order.total} đ</td>
                    <td>${order.status}</td>`;
...
function setupLoginForm() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (user.state === "Off") {
    // Nếu tìm thấy, NHƯNG state là "Off" -> Bị khóa
